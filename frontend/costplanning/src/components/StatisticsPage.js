                    ...
                    <Form onSubmit={changeForm}>
                        <Row>
                            <Col>
                                ...
                                <Form.Control as="select" onChange={methodForSetDate} value={selectedDate} >
                                    <option key={0}>Select...</option>
                                    {
                                        podaci.dates.map((p) => (<option key={p}>{p}</option>))
                                    }
                                </Form.Control>
                            </Col>
                            <Col>
                                <Button type="submit" variant="outline-info" block>Visualize</Button>
                            </Col>
                        </Row>
                    </Form>
                </div>
                <br></br>
                <PieChart lista={chartData.lista1} listaVrijednosti={chartData.lista2} selectedDate={selectedDate} ></PieChart>
            </div>)
    }
    else {
        return null
